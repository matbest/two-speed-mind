...
Delivered as a CLI chat app (`twospeed`) with slash commands.
...
- The slow/fast models are **interfaces** (`twospeed/interfaces.py`). Build and test everything
  against the **deterministic fakes** in `twospeed/fakes.py`. A real local model (llama.cpp / HF) is the
...
- `twospeed` (or `python -m twospeed`) — run the chat shell.
