...
def _render_wiki(store: Store, kainome: Path) -> None:
    ...
    for gene, page in store.clean.items():
        ...
        body = (
            ...
            f"{history}\n\n"
            f"*(generated by kaineros — edits here are overwritten)*\n"
        )
    ...
    rows = "\n".join(
        f"| [{page.gene}]({_safe(gene)}.md) | {page.content} |"
        for gene, page in store.clean.items()
    )
    index = (
        f"# Kainome — {len(store.clean)} page(s)\n\n"
        "The clean layer: every fact the mind currently believes, one page each.\n\n"
        + ("| page | says |\n|---|---|\n" + rows + "\n" if rows else "*(empty — nothing promoted yet)*\n")
        + "\n*(generated by kaineros — edits here are overwritten)*\n"
    )
