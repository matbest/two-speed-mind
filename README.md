...
**Kaineros** compiles experience into its **kainome** — a prototype of the **Two-Speed Mind** (White
...
The app is a command-line chat (`kaineros`) with slash commands, backed by the two-speed core.
...
kaineros          # start the chat shell (type /help)
```
...
The slow/fast models sit behind interfaces (`kaineros/interfaces.py`) so the whole pipeline is tested
with **deterministic fakes** (`kaineros/fakes.py`) — no real model needed to build or verify the
